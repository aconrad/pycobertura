...
def test_line_rate_by_class_file():
    ...
    for filename in cobertura.files():
        assert cobertura.line_rate(filename) == expected_line_rates[filename]
...
def test_branch_rate_by_class_file():
    ...
    for filename in cobertura.files():
        assert cobertura.branch_rate(filename) == expected_branch_rates[filename]
...
@pytest.mark.parametrize(
    "report, expected",
    [
        ("tests/cobertura-generated-by-istanbul-from-coffeescript.xml", ["app.coffee"]),
        (
            "tests/cobertura.xml",
            [
                "Main.java",
...
def test_total_statements_by_class_file():
    ...
    for filename in cobertura.files():
        assert (
            cobertura.total_statements(filename) == expected_total_statements[filename]
        )
...
def test_total_misses_by_class_file():
    ...
    for filename in cobertura.files():
        assert cobertura.total_misses(filename) == expected_total_misses[filename]
...
def test_total_hits_by_class_file():
    ...
    for filename in cobertura.files():
        assert cobertura.total_hits(filename) == expected_total_misses[filename]
...
def test_class_file_source__sources_not_found():
    ...
    expected_sources = {
        ...
        "search/BinarySearch.java": [
            Line(0, "tests/search/BinarySearch.java not found", None, None)
        ],
        "search/ISortedArraySearch.java": [
            Line(0, "tests/search/ISortedArraySearch.java not found", None, None)
        ],
        "search/LinearSearch.java": [
            Line(0, "tests/search/LinearSearch.java not found", None, None)
        ],
    }
    for filename in cobertura.files():
...
def test_line_statuses():
    ...
    expected_line_statuses = {
        ...
        "dummy/dummy4.py": [(1, False), (2, False), (4, False), (5, False), (6, False)],
    }
    for filename in cobertura.files():
        assert cobertura.line_statuses(filename) == expected_line_statuses[filename]


@pytest.mark.parametrize(
    "report, source, source_prefix",
    [
...
def test_class_file_source__raises_when_no_filesystem():
    ...
    for filename in cobertura.files():
        pytest.raises(Cobertura.MissingFileSystem, cobertura.file_source, filename)
...
def test_class_source_lines__raises_when_no_filesystem():
    ...
    cobertura = Cobertura("tests/cobertura.xml")
    for filename in cobertura.files():
        pytest.raises(Cobertura.MissingFileSystem, cobertura.source_lines, filename)
