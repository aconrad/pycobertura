...
def test_diff_class_source():
    ...
    expected_sources = {
        ...
        "dummy/dummy3.py": [
            ...
            Line(
                2,
                "    pass  # This is a very long comment that was purposefully written so we could test how HTML rendering looks like when the boundaries of the page are reached. And here is a non-ascii char: \u015e\n",
                False,
                "line-edit",
            ),
        ],
    ...
    for filename in cobertura2.files():
        assert differ.file_source(filename) == expected_sources[filename]
...
def test_diff_total_misses_by_class_file():
    ...
    for filename in cobertura2.files():
        assert differ.diff_total_misses(filename) == expected_sources[filename]
...
def test_diff_line_rate_by_class_file():
    ...
    for filename in cobertura2.files():
        assert differ.diff_line_rate(filename) == expected_sources[filename]
...
def test_diff_total_hits_by_class_file():
    ...
    for filename in cobertura2.files():
        assert differ.diff_total_hits(filename) == expected_total_hits[filename]
